...
class CrossSectionView extends Component {
  ...
  render() {
    ...
    return (
      <a-entity  >
        <a-entity click-rotation={`enabled:${clickRotation}`}  class={'clickable'} gltf-model={`url(${this.props.object})`} position={`${this.props.style.position[0]} ${this.props.style.position[1]} ${this.props.style.position[2]}`} play-all-model-animations pivot-center-model={`xPosition:${this.props.style.position[0]};yPosition:${this.props.style.position[1]};zPosition:${this.props.style.position[2]};pivotX:${this.props.style.xPivot};pivotY:${this.props.style.yPivot};pivotZ:${this.props.style.zPivot}`} material-modification={`specular:${specular};shininess:${shininess};emissive:${emissive};emphasisMaterial:${emphasisMaterial};emphasisMeshName:${meshName};emphasisOpacity:${emphasisOpacity};emphasisColor:${emphasisColor};modifyMaterial:${modifyMaterial};material:${material};materialColor:${materialColor};materialOpacity:${materialOpacity};edgeThresholdAngle:${edgeThresholdAngle};stroke:${stroke};strokeWidth:${strokeWidth};strokeColor:${strokeColor}`} highlight-on-click={`materialOpacity:${materialOpacity};emphasisOpacity:${emphasisOpacity};emphasisColor:${emphasisColor};materialColor:${materialColor};highlightEffect:${highlightEffect};highlightedOpacity:${highlightedOpacity};highlightedColor:${highlightedColor}`} scale={`${this.props.style.scale[0]} ${this.props.style.scale[1]} ${this.props.style.scale[2]}`} />  
        {animation}
      </a-entity>
    )
  }
