...
export default function (mapData, proj, scale, position, identifier, shapeKey) {
  ...
  switch (proj) {
    ...
    default: projection = false; break;
  }
  ...
  if(projection){
    ...
    features.forEach((d, i) => {
      if (!path(d))
        console.log(`Path doesn't exist in ${d}`)
      else
        featureNew.push(d)
    })
  ...
  if(proj){ 
    ...
    for (let i = 0; i < features.length; i++) {
      geoData.push({
        'code': features[i][identifier],
        'vertices': features[i]['geometry']['coordinates'],
      })
    }
  }
  return (geoData);
