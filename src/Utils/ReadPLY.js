...
export default function (fileName) {
  ...
  let rIndex = 0, gIndex = 0, bIndex = 0;
  ...
  for (let i = startIndex + 1; i < startIndex + vertexNo + 1; i++) {
    ...
    if (lines[rIndex + dataInfoStart].indexOf('red') !== -1)
      coordinates.push({ 'x': parseInt(coordinatesTemp[0], 10), 'y': parseInt(coordinatesTemp[1], 10), 'z': parseInt(coordinatesTemp[2], 10), 'r': parseInt(coordinatesTemp[rIndex], 10), 'g': parseInt(coordinatesTemp[gIndex], 10), 'b': parseInt(coordinatesTemp[bIndex], 10) })
    else
      coordinates.push({ 'x': parseInt(coordinatesTemp[0], 10), 'y': parseInt(coordinatesTemp[1], 10), 'z': parseInt(coordinatesTemp[2], 10) })
  }
  return (coordinates);
}
