  ...
  init: function() {
    if(this.data.stroke_bool){
      ...
      dataPoints.forEach(d => {
        outlinePnts.push(parseFloat(d[0].x),parseFloat(d[0].y),parseFloat(this.data.extrude))
        for(let  i = 1; i < d.length; i++){
          outlinePnts.push(parseFloat(d[i].x),parseFloat(d[i].y),parseFloat(this.data.extrude))
          outlinePnts.push(parseFloat(d[i].x),parseFloat(d[i].y),parseFloat(this.data.extrude))
        }
        outlinePnts.push(parseFloat(d[0].x),parseFloat(d[0].y),parseFloat(this.data.extrude))
      })
      ...
      let strokeVerFloat = new Float32Array(outlinePnts)
      lineGeometry.addAttribute( 'position', new THREE.BufferAttribute( strokeVerFloat, 3 ) );
