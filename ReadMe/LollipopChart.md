...
style.radius|object|Defines the radius of the mark. __Required.__ 
style.radius.scaleType|string|Defines the scale type for radius of the mark. __Not Required. If not present then a constant radius that is defined is used.__  _Available values: linear, ordinal._ 
style.radius.field|string|Defines the field in the data that will be mapped as radius of the mark. __Required if `style.radius.scaleType` is present.__
style.radius.value|float or array|Defines the radius of the mark. __Required.__ _If style.radius.scaleType is not present the this needs to be float otherwise an array of float._
